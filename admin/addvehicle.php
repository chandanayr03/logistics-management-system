...
if (isset($_REQUEST['license_plate'])){
        // removes backslashes
	$license_plate = stripslashes($_REQUEST['license_plate']);
	$license_plate = mysqli_real_escape_string($con,$license_plate);	
    $status = stripslashes($_REQUEST['status']);
	$status = mysqli_real_escape_string($con,$status);	
	
        $query = "INSERT into `vehicles` (license_plate,status)
VALUES ('$license_plate','$status')";
        $result = mysqli_query($con,$query);
        if($result=== TRUE){
           echo"Record inserted";?>
		<script type="text/javascript">
            window.alert("successfully Vehicle Added");
